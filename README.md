...
版本 1.4.0 新增“高级设置”功能，点击主界面“控制面板”区域的“高级设置”按钮即可进入。该面板主要用于精细化控制后台 Seed 服务（切片预缓存/更新服务）的行为。

#### 2.5.1 Seed 服务管理配置
*   **并发进程数 (Concurrency)**：设置 Seed 服务的并发进程数量。
    *   范围：1 - 16
    *   说明：根据服务器 CPU 核心数合理设置。默认值为 2。数值越高，切片生成速度越快，但 CPU 和内存占用越高。

#### 2.5.2 失败重试策略
*   **启用自动重试**：开关，决定当切片任务失败时是否自动重试。
*   **最大重试次数**：任务失败后的最大尝试次数（默认 2 次）。
*   **重试间隔**：每次重试前的等待时间（秒，默认 5 秒）。

#### 2.5.3 失败告警配置
*   **启用失败告警**：当任务在所有重试机会用尽后仍然失败时触发。
